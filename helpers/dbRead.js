 ...
 * @param {Object} dbRead - pg connection
 */
function Migrator (pgp, dbRead) {
	/**
	...
	this.checkMigrations = function (waterCb) {
		dbRead.one('SELECT to_regclass(\'migrations\')')
			.then(function (row) {
	...
	this.getLastMigration = function (hasMigrations, waterCb) {
		...
		dbRead.query('SELECT * FROM migrations ORDER BY "id" DESC LIMIT 1')
			.then(function (rows) {
				if (rows[0]) {
	...
	/**
	 * Creates and execute a dbRead query for each pending migration.
	 * @method
	...
	this.insertAppliedMigrations = function (appliedMigrations, waterCb) {
		async.eachSeries(appliedMigrations, function (file, eachCb) {
			dbRead.query('INSERT INTO migrations(id, name) VALUES($1, $2) ON CONFLICT DO NOTHING', [file.id.toString(), file.name])
				.then(function () {
					return eachCb();
				})
				.catch(function (err) {
...
module.exports.connect = function (config, logger, cb) {
	...
	let dbRead = pgp(config);

    //let migrator = new Migrator(pgp, dbRead);
    return cb(null, dbRead);
