...
DROP TRIGGER IF EXISTS rounds_fees_delete
ON "blocks";
DROP FUNCTION IF EXISTS round_fees_delete();
DROP TRIGGER IF EXISTS rounds_fees_insert
ON "blocks";
...
CREATE FUNCTION rounds_fees_init()
...
BEGIN
  ...
  WHERE height % 101 = 0 AND height NOT IN (SELECT height
                                            FROM rounds_fees)
  -- Group by round
  ...
  LOOP
    ...
    INSERT INTO rounds_fees
      SELECT
        ...
        (fees.single + (CASE WHEN last.pk = round."generatorPublicKey" AND last.timestamp = round.timestamp
...
CREATE INDEX IF NOT EXISTS "rounds_fees_timestamp"
  ON "rounds_fees" ("timestamp");
CREATE INDEX IF NOT EXISTS "rounds_fees_fees"
  ON "rounds_fees" ("fees");
CREATE INDEX IF NOT EXISTS "rounds_fees_height"
  ON "rounds_fees" ("height");
CREATE INDEX IF NOT EXISTS "rounds_fees_round"
  ON "rounds_fees" ((CEIL(height / 101 :: FLOAT) :: INT));
CREATE INDEX IF NOT EXISTS "rounds_fees_public_key"
  ON "rounds_fees" ("publicKey");

-- Create function for deleting round fees when last block of round is deleted
CREATE FUNCTION round_fees_delete()
  RETURNS TRIGGER LANGUAGE PLPGSQL AS $$
BEGIN
  DELETE FROM rounds_fees
  WHERE CEIL(height / 101 :: FLOAT) :: INT = (CEIL(OLD.height / 101 :: FLOAT) :: INT);
  RETURN NULL;
END $$;
...
CREATE FUNCTION round_fees_insert()
  RETURNS TRIGGER LANGUAGE PLPGSQL AS $$
BEGIN
  WITH
    -- Selecting all blocks of round
