...
class RoundRepository {
    ...
    setCurrentRound(round: Round): void {
        const roundId: number = !!this.prevRound.id ? this.prevRound.id + 1 : 1;
        this.currentRound = {id: roundId, ...round};
    }
    ...
    getLastSlotInRound(round: Round = this.currentRound): number {
        return Object.keys(round.slots)[Object.keys(round.slots).length - 1].slot;
    }
}
