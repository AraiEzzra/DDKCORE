...
DROP TRIGGER IF EXISTS protect_mem_accounts
ON "mem_accounts";

DROP FUNCTION IF EXISTS revert_mem_account();

CREATE FUNCTION revert_mem_account()
...
BEGIN
  ...
  IF NEW."u_username" <> OLD."u_username" AND OLD."u_username" IS NOT NULL
  THEN
  ...
  IF NEW."username" <> OLD."username" AND OLD."username" IS NOT NULL
  THEN
  ...
  IF NEW."virgin" <> OLD."virgin" AND OLD."virgin" = 0
  THEN
