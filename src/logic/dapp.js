...
DApp.prototype.verifyFields = function (trs, sender, cb) {
    ...
    if (trs.asset.dapp.icon) {
        ...
        const length = trs.asset.dapp.icon.length;
...
DApp.prototype.getBytes = function (trs) {
    ...
    try {
        buf = Buffer.from([]);
        const nameBuf = Buffer.from(trs.asset.dapp.name, 'utf8');
        buf = Buffer.concat([buf, nameBuf]);

        if (trs.asset.dapp.description) {
            const descriptionBuf = Buffer.from(trs.asset.dapp.description, 'utf8');
            buf = Buffer.concat([buf, descriptionBuf]);
        }

        if (trs.asset.dapp.tags) {
            const tagsBuf = Buffer.from(trs.asset.dapp.tags, 'utf8');
...
DApp.prototype.objectNormalize = function (trs) {
    for (const i in trs.asset.dapp) {
        if (trs.asset.dapp[i] === null || typeof trs.asset.dapp[i] === 'undefined') {
            delete trs.asset.dapp[i];
        }
    }

    const report = library.schema.validate(trs.asset.dapp, DApp.prototype.schema);

    if (!report) {
        throw `Failed to validate dapp schema: ${this.scope.schema.getLastErrors().map(err => err.message).join(', ')}`;
    }
