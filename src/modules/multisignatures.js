...
const schema = require('../schema/multisignatures.js');
const sql = require('../sql/multisignatures.js');
...
Multisignatures.prototype.processSignature = function (tx, cb) {
    const transaction = modules.transactions.getMultisignatureTransaction(tx.transaction);
...
Multisignatures.prototype.shared = {
    getAccounts(req, cb) {
        ...
        async.series({
            validateSchema(seriesCb) {
                library.schema.validate(req.body, schema.getAccounts, (err) => {
                    if (err) {
            ...
            getAccountIds(seriesCb) {
                library.db.one(sql.getAccountIds, { publicKey: req.body.publicKey }).then((row) => {
            ...
            getAccounts(seriesCb) {
                modules.accounts.getAccounts({
                    address: { $in: scope.accountIds },
                    sort: 'balance'
                }, ['address', 'balance', 'multisignatures', 'multilifetime', 'multimin'], (err, accounts) => {
                    if (err) {
            ...
            buildAccounts(seriesCb) {
                async.eachSeries(scope.accounts, (account, eachSeriesCb) => {
                    const addresses = [];

                    for (let i = 0; i < account.multisignatures.length; i++) {
                        addresses.push(modules.accounts.generateAddressByPublicKey(account.multisignatures[i]));
                    }

                    modules.accounts.getAccounts({
                        address: { $in: addresses }
                    }, ['address', 'publicKey', 'balance'], (err, multisigaccounts) => {
                        if (err) {
    ...
    pending(req, cb) {
        ...
        async.series({
            validateSchema(seriesCb) {
                library.schema.validate(req.body, schema.pending, (err) => {
                    if (err) {
                        return setImmediate(seriesCb, err[0].message);
                    }
                    return setImmediate(seriesCb);
            ...
            getTransactionList(seriesCb) {
                scope.transactions = modules.transactions.getMultisignatureTransactionList(false, false);
                scope.transactions = scope.transactions.filter(transaction => transaction.senderPublicKey === req.body.publicKey);

                return setImmediate(seriesCb);
            },
            buildTransactions(seriesCb) {
                async.eachSeries(scope.transactions, (transaction, eachSeriesCb) => {
                    let signed = false;

                    if (transaction.signatures && transaction.signatures.length > 0) {
                        let verify = false;

                        for (const i in transaction.signatures) {
                            try {
        ...
        }, err => setImmediate(cb, err, { transactions: scope.pending }));
    },
    ...
    sign(req, cb) {
        ...
        function checkTransactionPermissions(cb) {
            ...
            const alreadySigned = (scope.transaction.signatures && scope.transaction.signatures.indexOf(scope.signature) !== -1);
        ...
        library.balancesSequence.add((cb) => {
            async.series({
                validateSchema(seriesCb) {
                    library.schema.validate(req.body, schema.sign, (err) => {
                        if (err) {
                            return setImmediate(seriesCb, err[0].message);
                        }
                        return setImmediate(seriesCb);
                ...
                const transaction = modules.transactions.getMultisignatureTransaction(req.body.transactionId);
    ...
    addMultisignature(req, cb) {
        ...
        library.balancesSequence.add((cb) => {
            async.series({
                validateSchema(seriesCb) {
                    library.schema.validate(req.body, schema.addMultisignature, (err) => {
                        if (err) {
                            return setImmediate(seriesCb, err[0].message);
                        }
                        return setImmediate(seriesCb);
                ...
                addMultisignature(seriesCb) {
                    ...
                    modules.accounts.setAccountAndGet({ publicKey: scope.keypair.publicKey.toString('hex') }, (err, account) => {
                        if (err) {
                            return setImmediate(seriesCb, err);
                        }
