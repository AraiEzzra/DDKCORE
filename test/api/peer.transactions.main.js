...
describe('POST /peer/transactions', function () {
	...
	it('using valid headers should be ok', function (done) {
		...
		let transaction = node.ddk.transaction.createTransaction(account.address, 1, node.gAccount.password);
	...
	it('using varying recipientId casing should go to same address', function (done) {
		let account = node.randomAccount();
		let keys = node.ddk.crypto.getKeys(account.password);
		let address = node.ddk.crypto.getAddress(keys.publicKey);

		let transaction = node.ddk.transaction.createTransaction(address, 100000000, node.gAccount.password);
		postTransaction(transaction, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.ok;

			node.onNewBlock(function (err) {
				let transaction2 = node.ddk.transaction.createTransaction(address.toLowerCase(), 100000000, node.gAccount.password);
	...
	it('using transaction with undefined recipientId should fail', function (done) {
		let transaction = node.ddk.transaction.createTransaction(undefined, 1, node.gAccount.password);
	...
	it('using transaction with invalid recipientId should fail', function (done) {
		let transaction = node.ddk.transaction.createTransaction('0123456789001234567890L', 1, node.gAccount.password);
	...
	it('using transaction with negative amount should fail', function (done) {
		let transaction = node.ddk.transaction.createTransaction('1L', -1, node.gAccount.password);
	...
	it('using invalid passphrase should fail', function (done) {
		let transaction = node.ddk.transaction.createTransaction('12L', 1, node.gAccount.password);
	...
	it('when sender has no funds should fail', function (done) {
		let transaction = node.ddk.transaction.createTransaction('1L', 1, 'randomstring');
	...
	it('when sender does not have enough funds should always fail', function (done) {
		...
		postTransaction(transaction, function (err, res) {
			...
			node.onNewBlock(function () {
				let count = 1;
				let transaction2 = node.ddk.transaction.createTransaction(node.gAccount.address, 2, account.password);
	...
	it('using very large amount and genesis block id should fail', function (done) {
		let transaction = node.ddk.transaction.createTransaction('12L', 10000000000000000, node.gAccount.password);
	...
	it('using overflown amount should fail', function (done) {
		let transaction = node.ddk.transaction.createTransaction('12L', 184819291270000000012910218291201281920128129,
	...
	it('using float amount should fail', function (done) {
		let transaction = node.ddk.transaction.createTransaction('12L', 1.3, node.gAccount.password);
	...
	describe('from the genesis account', function () {
		...
		let signedTransactionFromGenesis = {
			type: 0,
			amount: 1000,
