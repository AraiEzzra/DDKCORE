...
describe('Test GET_TRANSACTIONS_BY_BLOCK_ID', () => {
    ...
    it('Success test GET_TRANSACTIONS_BY_BLOCK_ID', (done) => {
        const SUCCESS_TRANSACTIONS = [{
            ...
            'signature': 'a1e5da01e33df8501558bcb892050d8bf64dce442c5d5875b777b3e82fcfb339ea'
                + '5ff6a9b06adf0bd58fb7a866c5adde9561e903391b58e6c1992e711100a709',
            'secondSignature': null,
            'fee': 0,
            'salt': '35a841ed3dc14886cd1757837908c8b3',
            'asset': { 'recipientAddress': '933553974927686133', 'amount': 171000000000000 }
        }, {
            ...
            'signature': '226ed984bf3d82b7c332ce48bc976fcc35930d22cb068b2e9de993a4fb3e402d4bdb7077' +
                'd0923b8dd2c205e6a2473884752615c0787967b218143eec5df1390c',
            'secondSignature': null,
            'fee': 0,
            'salt': 'a7fdae234eeb416e31f5f02571f54a0c',
            'asset': { 'recipientAddress': '4995063339468361088', 'amount': 4500000000000000 }
        }];
        ...
        socket.on('message',
            (response: Message2<ResponseEntity<{ transactions: Array<SerializedTransaction<IAsset>>, count: number }>>
                ...
                if (response.headers.id === HEADERS.id) {
                    console.log(JSON.stringify(response.body.data.transactions));
    ...
    it('Negative test GET_TRANSACTIONS_BY_BLOCK_ID', (done) => {
        const HEADERS = Fixture.getBaseHeaders();
