...
/**
 * Check User unconfirmed transaction List
 * @param {json} req
 * @return {function} Calls __private.getPooledTransactions
 */
Transactions.prototype.getUserUnconfirmedTransactions = function (method, req, cb) {
	return __private.getPooledTransactions(method, req, cb);
};

Transactions.prototype.getLastTransactionConfirmations = function (senderId, cb) {
	library.db.query(sql.getLastTransactionConfirmations, {
		senderId: senderId
	})
		.then(function (data) {
			return setImmediate(cb, null, data);
